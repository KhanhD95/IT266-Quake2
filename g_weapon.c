...
void fire_blaster (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	...
	bolt->movetype = MOVETYPE_FLYRICOCHET; // was flymissle
...
static void Cluster_Explode(edict_t *ent)
{
	...
	fire_grenade2(ent, origin, grenade1, 120, 10, 1.0, 120,held);
	fire_grenade2(ent, origin, grenade1, 120, 10, 1.0, 120,held);
	fire_grenade2(ent, origin, grenade1, 120, 10, 1.0, 120,held);
	fire_grenade2(ent, origin, grenade1, 120, 10, 1.0, 120,held);
...
void fire_grenade2 (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
	...
	grenade->touch = Grenade_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = Cluster_Explode; // cluster grenade
	grenade->dmg = damage;
	grenade->dmg_radius = damage_radius;
...
// Think function for homing missiles
...
void fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	//Check if this a player and if they have HS on
	if (self->client&&self->client->pers.homing_state)
	{
		//if there are 5 cells, start homing, otherwise normal rockets
